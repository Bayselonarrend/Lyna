...
Функция ПолучитьОбъектКомпоненты() Экспорт
	...
	Попытка
		...
		ТекущийПуть.Добавить("Luna.zip");

		ПодключитьВнешнююКомпоненту(СтрСоединить(ТекущийПуть, "/"), "Luna", ТипВнешнейКомпоненты.Native);

		ОбъектКомпоненты = Новый("AddIn.Luna.Main");
