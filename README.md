...
**Lyna** - это библиотека для интеграции языка программирования Lua с OneScript. Она представляет из себя Native API компоненту на языке Rust, в основе которой лежит крейт [`mlua`](https://github.com/mlua-rs/mlua), а также обертку вокруг нее. Цель Lyna - дать разработчику возможность увеличить скорость выполнения своих скриптов и решений, вынеся часть тяжелой логики на более производительный движок Lua, не отказываясь и не переходя с платформы OneScript полностью.
